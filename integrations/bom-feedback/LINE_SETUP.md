...
- 只打「#回報」→ 機器人回選單（🐞 回報問題／💡 提出需求／🗂️ 更新資料，不建立回報）；選問題或需求後，同一人 10 分鐘內的下一句就是描述（需求在「問題類型」欄記為 LINE 需求），建立編號後才收截圖。
- 只打「#更新」→ 機器人回資料類型按鈕選單（不建立需求）；按鈕或直接打「#更新 PN_Project_Map（說明）」→ 機器人回「收到 U001」；同一人 30 分鐘內傳的 Excel（.xlsx／.xls，最多 3 個、每個 10 MB 內）存入私人 Drive。
...
| Q | 回覆時間 | 自動 |
| R／S | 同事補充／補充時間 | 同事打「F005 ②」這類「編號＋文字」時自動記錄並回覆「收到 F005 的補充」；Data Requests 對應 L／M 欄 |
